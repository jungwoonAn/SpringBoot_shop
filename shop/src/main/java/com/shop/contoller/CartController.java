...
public class CartController {
    ...
    public @ResponseBody ResponseEntity order(@RequestBody @Valid CartItemDto cartItemDto,
        ...
        if(bindingResult.hasErrors()) {  // 객체 바인딩시 에러 검사
        ...
        // 생성된 장바구니 상품 아이디, 요청 성공코드 반환
        return new ResponseEntity<Long>(cartItemId, HttpStatus.OK);
    }

    @GetMapping(value = "/cart")
    public String orderHist(Principal principal, Model model){
        // 현재 로그인한 사용자 이메일 정보로 장바구니에 담겨있는 상품 조회
    ...
    // 요청된 자원의 일부를 업데이트할 때에는 PATCH 사용
    ...
    public @ResponseBody ResponseEntity updateCartItem(
        ...
        if(count <= 0){  // 상품 갯수가 0개 이하로 요청할 때
    ...
    public @ResponseBody ResponseEntity deleteCartItem(
        ...
        if(!cartService.validateCartItem(cartItemId, principal.getName())){  // 삭제 권한 체크
    ...
    public @ResponseBody ResponseEntity orderCartItem(@RequestBody CartOrderDto cartOrderDto,
        ...
        if(cartOrderDtoList == null || cartOrderDtoList.size() == 0){  // 주문할 상품을 선택하지 않았으면
